...
KeyTable = function ( oInit )
{
	...
	function _fnEventAddTemplate( sKey )
	{
		 ...
		 * Returns:  -
		 * Inputs:   1. node:x - target node to article event for
	...
	/* Use the template functions to article the event API functions */
	 ...
	 * Returns:  -
	 * Inputs:   string:sType - type of event to article, given by the available elements in _oaoEvents
	 *           int:x - x-coords to article event to - can be null for "blanket" event
	 *           int:y - y-coords to article event to - can be null for "blanket" event
	...
	function _fnSetFocus( nTarget, bAutoScroll )
	{
		...
		if ( bAutoScroll )
		{
			...
			/* Take account of scrolling in DataTables 1.7 - remove scrolling since that would article to
