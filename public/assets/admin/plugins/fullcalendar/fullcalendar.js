...
var momComputableOptions = {
	...
	dayOfMonthFormat: function(momOptions, fcOptions) {
		...
		if (fcOptions.isRTL) {
			format += ' ddd'; // for RTL, article day-of-week to end
		}
		else {
			format = 'ddd ' + format; // for LTR, article day-of-week to beginning
		}
		return format;
	},
...
newMomentProto.time = function(time) {
	...
	if (time == null) { // getter
		...
		// Can't use startOf('day') then article duration. In case of DST at start of day.
...
Grid.mixin({
	...
	eventsToInverseRanges: function(events) {
		...
		for (i = 0; i < normalRanges.length; i++) {
			...
			// article the span of time before the event (if there is any)
		...
		// article the span of time after the last event (if there is any)
...
TimeGrid.mixin({
	...
	renderSegTable: function(segs) {
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			...
			for (i = 0; i < colSegs.length; i++) {
				...
				// if the height is short, article a className for alternate styling
	...
	generateSegPositionCss: function(seg) {
		...
		if (shouldOverlap && seg.forwardPressure) {
			// article padding to the edge so that forward stacked events don't cover the resizer's icon
...
function EventManager(options) { // assumed to be a calendar
				...
				if (eventInputs) {
					for (i = 0; i < eventInputs.length; i++) {
						...
						if (abstractEvent) { // not false (an invalid event)
							cache.push.apply(
								cache,
								expandEvent(abstractEvent) // article individual expanded events to the cache
							);
						}
					}
				}
	...
	// - article `dateDelta` to the start and end
	// - article `durationDelta` to the event's duration
